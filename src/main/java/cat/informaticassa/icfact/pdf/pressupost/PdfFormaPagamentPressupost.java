...
public class PdfFormaPagamentPressupost {
    public void afegir (Document document, Pressupost pressupost, Empresa empresa) {
        try {
            ...
            if (pressupost.getFormaPagament() != null) {
                document.add(new Paragraph("Forma: " + pressupost.getFormaPagament().getNom(), normal));
                document.add(new Paragraph(pressupost.getFormaPagament().getDescripcio(), normal));
                if (pressupost.getFormaPagament().getMostrarIban()) {
                    document.add(new Paragraph("IBAN: " + empresa.getIban(), normal));
                }
            }
        } catch (Exception e) {
