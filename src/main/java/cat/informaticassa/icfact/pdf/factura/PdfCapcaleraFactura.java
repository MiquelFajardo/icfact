...
public class PdfCapcaleraFactura {
    ...
    public void afegir(Document document, Factura factura, Empresa empresa) {
        try {
            ...
            PdfPCell textTitol = new PdfPCell(new Phrase("FACTURA", PdfFonts.titolGran(colorEmpresa)));
            ...
            dades.addCell(new Phrase(PdfUtils.formatData(factura.getData()), subtitol));
            dades.addCell(new Phrase("Client", PdfFonts.etiqueta(colorEmpresa)));
            dades.addCell(new Phrase(factura.getClient().getNom(), subtitol ));
            dades.addCell(new Phrase( "NIF", PdfFonts.etiqueta(colorEmpresa)));
            dades.addCell(new Phrase(factura.getClient().getNif(), subtitol));
            dades.addCell(new Phrase("Adreça", PdfFonts.etiqueta(colorEmpresa)));
            dades.addCell(new Phrase(factura.getClient().getAdreca().getAdrecaCompleta(), normal));

            String codiPostal = "";
            String poblacio = "";
            if (factura.getClient().getAdreca().getPoblacio() != null) {
                poblacio = factura.getClient().getAdreca().getPoblacio().getNom();
                codiPostal = factura.getClient().getAdreca().getPoblacio().getCodiPostal().stream().findFirst().orElse("");
            }
            ...
            dades.addCell(new Phrase("Població", PdfFonts.etiqueta(colorEmpresa)));
            dades.addCell(new Phrase(cpPoblacio, normal));

            if (factura.getClient().getTelefon() != null && !factura.getClient().getTelefon().isBlank()) {
                dades.addCell(new Phrase("Telèfon", PdfFonts.etiqueta(colorEmpresa)));
                dades.addCell(new Phrase(factura.getClient().getTelefon(), normal ));
            }
            ...
            String codiPostalEmpresa = "";
            String poblacioEmpresa = "";

            if (empresa.getAdreca().getPoblacio() != null) {
                poblacioEmpresa = empresa.getAdreca().getPoblacio().getNom();
                codiPostalEmpresa = empresa.getAdreca()
                        .getPoblacio()
                        .getCodiPostal()
                        .stream()
                        .findFirst()
            ...
            String cpPoblacioEmpresa = codiPostalEmpresa;
            if (!poblacioEmpresa.isBlank()) {
                if (!cpPoblacioEmpresa.isBlank()) {
                    cpPoblacioEmpresa += " - ";
                }
                cpPoblacioEmpresa += poblacioEmpresa;
            }

            dadesEmpresa.addCell(new Phrase(cpPoblacioEmpresa, normal));
