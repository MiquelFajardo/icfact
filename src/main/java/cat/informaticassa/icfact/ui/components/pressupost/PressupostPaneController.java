...
public class PressupostPaneController {
    ...
    public PressupostPaneController(PressupostPane pane) {
        this.pane = pane;
        inicialitzar();
    }
    ...
    private void inicialitzar() {
        ...
        pane.getTxtClient().setSearchFunction(client ->
                client.getNom()
                        ...
                        + " "
                        + client.getTelefon());
        pane.getCmbFormaPagament().setConverter(new javafx.util.StringConverter<>() {
    ...
    private void carregarClients() {
        pane.getTxtClient().setItems(
                buscarClientsService.buscarTots()
        );
    }

    private void carregarFormesPagament() {
        pane.getCmbFormaPagament().getItems().setAll(
                buscarFormesPagamentService.buscarActius()
        );
    }
